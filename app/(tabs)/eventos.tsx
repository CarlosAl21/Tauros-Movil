...
export default function EventsScreen() {
      ...
      <TaurosScreen>
        <View style={styles.topIntro}>
          ...
          <Text style={styles.pageSubtitle}>
            Inicia sesión para consultar y registrarte en eventos activos.
          </Text>
        </View>
        <TaurosAuthCard />
      </TaurosScreen>
  ...
  const displayEvents = mapBackendEvents(events, user?.userId).filter(
    (event) => event.activo,
  );
  ...
  return (
    <TaurosScreen>
      ...
      <TaurosSection title="Eventos activos">
        {displayEvents.length ? (
                ...
                <View style={styles.eventTopRow}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.eventMeta}>
                      {new Date(event.fechaHora).toLocaleDateString("es-EC")} ·{" "}
                      {event.lugar}
                    </Text>
                  </View>
                  ...
                  <TaurosButton
                    ...
                    onPress={async () => {
                      try {
                        ...
                        Alert.alert(
                          "Registro exitoso",
                          "Te registraste correctamente en el evento.",
                        );
                      } catch (error) {
                        Alert.alert(
                          "Registro",
          ...
          <TaurosCard>
            <TaurosEmptyState message="No hay eventos activos disponibles en este momento." />
          </TaurosCard>
        )}
      </TaurosSection>
    </TaurosScreen>
